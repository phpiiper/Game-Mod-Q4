...
// PLP_MOD :: blaster only hitscan hit?
			idPlayer* player = GetLocalPlayer();
			if (player && !idStr::Icmp(ent->GetEntityDefName(), "monster_grunt") && (owner && owner == static_cast<idEntity*>(player))) {
				...
				if (currentWeapon == blaster && InRound) {
					// PLP_MOD :: blaster only hitscan hit?
					if (currentWeapon == blaster) {
						...
						for (int i = 0; i < MAX_RUNES; i++) {
							if (player->inventory.runes[i] == 1) {
								...
								if (def) {
									int runeType = def->dict.GetInt("type", 0);
									int runeLevel = def->dict.GetInt("level", 0);
									if (runeType == 0) { burnLevel = runeLevel; }
									if (runeType == 1) { freezeLevel = runeLevel; }
									if (runeType == 2) { uppieLevel = runeLevel; }
									if (runeType == 3) { 
										atkMult += 0.5 * runeLevel; // only stackable damage
									}
								}
							}
						}
						// IF runes are active... (and ent is Monster_[name])... do thing here
						activateBurn = (random.RandomInt(100) > 100 - 10 * burnLevel);
						burnPercentHP = 1 * burnLevel;
						activateFreeze = (random.RandomInt(100) > 100 - 10 * freezeLevel);
						activateUppie = (random.RandomInt(100) > 100 - 10 * uppieLevel);
						Printf("7903 :: Burn %s | Freeze %s | Uppie %s \n",activateBurn ? "ON" : "OFF", activateFreeze ? "ON" : "OFF", activateUppie ? "ON" : "OFF");
						// Burn
						if (burnLevel > 0 && activateBurn) {
							if (ai) { ai->PLPBurn(burnLevel); }
							else {
								Printf("A::AI not found...\n");
							}
						}
						// Freeze
						if (freezeLevel > 0 && activateFreeze) {
							if (ai) { ai->PLPFreeze(freezeLevel); }
							else {
								Printf("B::AI not found...\n");
							}
						}
						// Uppie
						if (uppieLevel > 0 && activateUppie) {
							Printf("7918 --- Activate Wind\n");
							idPhysics* phys = ent->GetPhysics();
							if (phys) {
								idVec3 vel = phys->GetLinearVelocity();
								vel.z += 300.0f * (uppieLevel + 1);
								phys->SetLinearVelocity(vel);
							}
						}
						if (atkMult > 1) {
							Printf("Attack Multiplier Effect Applied\n");
							damageScale += atkMult;
						}
						// DAMAGE MODIFIER as ELEMENTS
						if (ai && ai->PLPType != -1) {
							Printf(">> Damage Modifier Applied...\n");
							if (burnLevel > 0 && ai->PLPType == 2) { // Fire Rune used ; Against ICE Elemental (STR)
								damageScale *= 2.0;
								Printf("[DM] Fire > Ice (2x)\n");
							}
							if (burnLevel > 0 && ai->PLPType == 3)  { // Fire Rune used ; Against WIND Elemental (WK)
								damageScale *= 0.5;
								Printf("[DM] Fire > Wind (0.5x)\n");
							}
							if (freezeLevel > 0 && ai->PLPType == 3)  { // Ice Rune used ; Against WIND Elemental (STR)
								damageScale *= 2.0;
								Printf("[DM] Ice > Wind (2x)\n");
							}
							if (freezeLevel > 0 && ai->PLPType == 1) { // Ice Rune used ; Against FIRE Elemental (WK)
								damageScale *= 0.5;
								Printf("[DM] Ice > Fire (0.5x)\n");
							}
							if (uppieLevel > 0 && ai->PLPType == 1) { // Wind Rune used ; Against FIRE Elemental (STR)
								damageScale *= 2.0;
								Printf("[DM] Wind > Fire (2x)\n");
							}
							if (uppieLevel > 0 && ai->PLPType == 2)  { // Wind Rune used ; Against ICE Elemental (WK)
								damageScale *= 0.5;
								Printf("[DM] Wind > Ice (0.5x)\n");
							}
						}
					}
				}
