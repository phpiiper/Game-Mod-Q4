...
void idPlayer::Init(void) {
	...
	*/
	...
	inventory.RUNE_DEF_NAMES[0] = "item_rune_fire_i";
	inventory.RUNE_DEF_NAMES[1] = "item_rune_fire_ii";
	inventory.RUNE_DEF_NAMES[2] = "item_rune_fire_iii";

	inventory.RUNE_DEF_NAMES[3] = "item_rune_ice_i";
	inventory.RUNE_DEF_NAMES[4] = "item_rune_ice_ii";
	inventory.RUNE_DEF_NAMES[5] = "item_rune_ice_iii";

	inventory.RUNE_DEF_NAMES[6] = "item_rune_air_i";
	inventory.RUNE_DEF_NAMES[7] = "item_rune_air_ii";
	inventory.RUNE_DEF_NAMES[8] = "item_rune_air_iii";

	inventory.RUNE_DEF_NAMES[9] = "item_rune_attack_i";
	inventory.RUNE_DEF_NAMES[10] = "item_rune_attack_ii";
	inventory.RUNE_DEF_NAMES[11] = "item_rune_attack_iii";
...
void idPlayer::Spawn(void) {
	...
	if (!gameLocal.isMultiplayer || entityNumber == gameLocal.localClientNum) {
		...
		// PLPMOD
		plpRuneGUI = uiManager->FindGui("guis/plpRuneGUI.gui", true, false, true);
		if (!plpRuneGUI) {
			gameLocal.Warning("idPlayer::Spawn() - Failed to load plpRuneGUI");
		}
...
void idPlayer::UpdateHudStats(idUserInterface* _hud) {
	...
	// PLPMOD 3458
	_hud->SetStateFloat("InRound", InRound);
	_hud->SetStateInt("CurrentRound", CurrentRound);
	_hud->SetStateInt("EnemiesLeft", EnemiesLeft);
	_hud->HandleNamedEvent("updatePLPHud");
...
// mekberg: adjustable crosshair size.
		...
		// PLP :: plpCustomHud
		if (showPlpRuneGUI && plpRuneGUI) {
			plpRuneGUI->Redraw(gameLocal.time);
			RouteGuiMouse(plpRuneGUI);
		}
	}
...
void idPlayer::GiveItem(const char* itemname) {
	...
	if (!idStr::Icmpn(itemname, "item_rune_", 10)) {
		...
		if (def) {
			...
			inventory.RUNE_DEF_NAMES[runeId] = itemname;
		}
...
void idPlayer::Weapon_GUI(void) {
	...
	if (!objectiveSystemOpen && !showPlpRuneGUI) {
		if (idealWeapon != currentWeapon) {
			Weapon_Combat();
		}
		StopFiring();
...
void idPlayer::UpdateViewAngles(void) {
	...
	if (!noclip && (gameLocal.inCinematic || privateCameraView || gameLocal.GetCamera() || influenceActive == INFLUENCE_LEVEL2 || showPlpRuneGUI)) {
...
void idPlayer::RouteGuiMouse(idUserInterface * gui) {
	...
	if (usercmd.mx != oldMouseX || usercmd.my != oldMouseY) {
		...
		if (plpRuneGUI && showPlpRuneGUI){
			command = plpRuneGUI->HandleEvent(&ev, gameLocal.time);
		}
		else {
...
void idPlayer::StartRound()
{
	if (!InRound) {
		...
		idVec3 origin = idVec3(-591.54f, 962.72f, -2040.16f);
		idAngles angle = idAngles();
		Teleport(origin, angle, NULL);
		UpdateHud();
		PLPUpdateHUD();
		SpawnWave();
...
{
	int SPAWNCOUNT = 3 + (CurrentRound % 5 * 2);
...
void idPlayer::SpawnRandomEnemy()
{
	...
	float angle = gameLocal.random.RandomFloat() * 360.0f;
	float distance = 750.0f + gameLocal.random.RandomFloat() * 500.0f;

	idVec3 spawnPos;
	spawnPos.x = playerOrigin.x + idMath::Cos(DEG2RAD(angle)) * distance;
	spawnPos.y = playerOrigin.y + idMath::Sin(DEG2RAD(angle)) * distance;
	spawnPos.z = playerOrigin.z;
	...
	gameLocal.TracePoint(this, tr, start, end, MASK_SHOT_BOUNDINGBOX, this);
	if (tr.fraction < 1.0f) {
		spawnPos = tr.endpos + idVec3(0, 0, 1);
	}

	idDict args;
	args.Set("classname", "monster_grunt");
	args.SetVector("origin", spawnPos);
	args.SetMatrix("rotation", mat3_identity);
...
{
	if (InRound) {
		gameLocal.Printf("[PLP] Round ENDED\n");
		InRound = false;
		// ToggleGUI();
		UpdateHud();
		PLPUpdateHUD();
...
void idPlayer::ToggleGUI() {
	if (!plpRuneGUI) {
		gameLocal.Printf("plpRuneGUI :: is NULL\n");
		return;
	}
	if (!showPlpRuneGUI) {
		showPlpRuneGUI = !showPlpRuneGUI;
		PLPUpdateHUD();
		plpRuneGUI->StateChanged(gameLocal.time);
		plpRuneGUI->Activate(true, gameLocal.time);
		plpRuneGUI->Redraw(gameLocal.time);
		plpRuneGUI->HandleNamedEvent("updatePLP");
		plpRuneGUI->HandleNamedEvent("showPLPRuneShow");
	}
	else {
		showPlpRuneGUI = !showPlpRuneGUI;
		PLPUpdateHUD();
		plpRuneGUI->StateChanged(gameLocal.time);
		plpRuneGUI->Activate(false, gameLocal.time);
		plpRuneGUI->Redraw(gameLocal.time);
		plpRuneGUI->HandleNamedEvent("updatePLP");
		plpRuneGUI->HandleNamedEvent("showPLPRuneHide");
	}
}
void idPlayer::PLPUpdateHUD()
{
	if (hud) {
		//gameLocal.Printf("[PLP] [UPDATE-HUD] :: updating... \n");
		// gameLocal.Printf("[PLP] [UPDATE-HUD] :: InRound = %s ; CurrentRound = %d ; EnemiesLeft = %d... \n",InRound ? "true" : "false", CurrentRound, EnemiesLeft);
		hud->SetStateFloat("InRound", InRound);
		hud->SetStateString("InRoundText", InRound ? "IN-ROUND" : "OUT-ROUND");
		hud->SetStateInt("CurrentRound", CurrentRound);
		hud->SetStateInt("EnemiesLeft", EnemiesLeft);
		hud->SetStateString("EnemiesLeftText", "enemies left: " + EnemiesLeft);
		hud->StateChanged(gameLocal.time);
		hud->Redraw(gameLocal.time);
		hud->HandleNamedEvent("updatePLPHud");
	}
	if (showPlpRuneGUI) {
		plpRuneGUI->SetStateString("InRoundText", InRound ? "IN-ROUND" : "OUT-ROUND");
		plpRuneGUI->SetStateFloat("inRound", InRound);
		plpRuneGUI->SetStateInt("Round", CurrentRound);
		plpRuneGUI->SetStateInt("SkillPoints", SkillPoints);
		plpRuneGUI->SetStateInt("EnemiesLeft", EnemiesLeft);



		plpRuneGUI->StateChanged(gameLocal.time);
		plpRuneGUI->Redraw(gameLocal.time);
		plpRuneGUI->HandleNamedEvent("updatePLPUI");
...
bool idPlayer::ToggleRune(const char* name)
{
	if (HasRune(name)) {
		const idDeclEntityDef* def = gameLocal.FindEntityDef(name, false);
		int runeId = def->dict.GetInt("id", "-1");
		gameLocal.Printf("[PLPMOD] [RM] 14560 Toggling [Rune] ID %d ... \n", runeId);
		if (inventory.runes[runeId] == 0) { return false; }
		...
		plpRuneGUI->SetStateFloat(va("activated_%s", name), inventory.runes[runeId]);
		plpRuneGUI->StateChanged(gameLocal.time);
		plpRuneGUI->HandleNamedEvent("updatePLPUI");
		return true;
	}
...
bool idPlayer::GiveRune(const char* name) {
	const idDeclEntityDef* def = gameLocal.FindEntityDef(name, false);
	if (!def) {
		gameLocal.Printf("[GiveRune] DEF NOT FOUND for '%s'\n", name);
		return false;
	}

	int runeId = def->dict.GetInt("id", "-1");
	gameLocal.Printf("[GiveRune] runeId=%d, runes[runeId]=%d\n", runeId, inventory.runes[runeId]);

	if (runeId != -1 && inventory.runes[runeId] == 0) {
		inventory.runes[runeId] = -1;
		if (plpRuneGUI) {
			const char* varName = va("bought_%s", name);
			gameLocal.Printf("[GiveRune] Setting GUI var '%s' to 1.0\n", varName);
			plpRuneGUI->SetStateFloat(varName, 1.0f);
			float check = plpRuneGUI->GetStateFloat(varName);
			gameLocal.Printf("[GiveRune] Readback value = %f\n", check);
			plpRuneGUI->StateChanged(gameLocal.time);
			plpRuneGUI->HandleNamedEvent("updatePLPUI");
			gameLocal.Printf("[GiveRune] Done\n");
		}
		else {
			gameLocal.Printf("[GiveRune] plpRuneGUI IS NULL\n");
		}
		return true;
	}
	return false;
...
bool idPlayer::AddRune(const char* name)
{
	if (HasRune(name)) {
		const idDeclEntityDef* def = gameLocal.FindEntityDef(name, false);
		int runeId = def->dict.GetInt("id", "-1");
		gameLocal.Printf("[PLPMOD] [RM] Add Rune w/ ID %d ... \n", runeId);
		inventory.runes[runeId] = 1;
		plpRuneGUI->SetStateFloat(va("activated_%s", name), 1);
		plpRuneGUI->StateChanged(gameLocal.time);
		plpRuneGUI->Redraw(gameLocal.time);
		plpRuneGUI->HandleNamedEvent("updatePLPUI");
		return true;
...
bool idPlayer::RemoveRune(const char* name)
{
	if (HasRune(name)) {
		const idDeclEntityDef* def = gameLocal.FindEntityDef(name, false);
		int runeId = def->dict.GetInt("id", "-1");
		gameLocal.Printf("[PLPMOD] [RM] Remove Rune w/ ID %d ... \n", runeId);
		inventory.runes[runeId] = -1;
		plpRuneGUI->SetStateFloat(va("activated_%s", name), -1);
		plpRuneGUI->StateChanged(gameLocal.time);
		plpRuneGUI->Redraw(gameLocal.time);
...
bool idPlayer::BuyRune(const char* name)
{
	// Check if rune exists
	const idDeclEntityDef* def = gameLocal.FindEntityDef(name, false);
	if (def) {
		gameLocal.Printf("[PLPRM] [BUY RUNE] 1 :: DEF FOUND \n");
		int runeId = def->dict.GetInt("id", "-1");
		if (runeId != -1 && inventory.runes[runeId] == 0) {
			// runeId is valid AND rune is unowned, attempt buy
			// FIRST check if other rune upgrade is valid???
			// :: if lvl = 1, pass, else if ID-1 == unowned, return false
			gameLocal.Printf("[PLPRM] [BUY RUNE] 2 :: RUNE UNOWNED, FOUND \n");
			if (SkillPoints - 1 >= 0) {
				gameLocal.Printf("[PLPRM] [BUY RUNE] 3 :: CAN BUY \n");
				// Can only buy if SkillPoints > 0
				if (GiveRune(name)) {
					gameLocal.Printf("[PLPRM] [BUY RUNE] 4 :: GIVEN RUNE \n");
					SkillPoints -= 1;
					return true;
				};
				return false;
	...
	gameLocal.Printf("[PLPRM] [BUY RUNE] 1 :: DEF NOT FOUND \n");
	return false;
}
