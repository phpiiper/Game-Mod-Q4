...
void rvWeaponRocketLauncher::Spawn ( void ) {
	...
	reloadRate = SEC2MS ( spawnArgs.GetFloat ( "reloadRate", ".2" ) );
	
	guideAccelTime = SEC2MS ( spawnArgs.GetFloat ( "lockAccelTime", ".1" ) );
	...
	if ( animNum ) {
		...
		rate = (float)anim->Length() / (float)SEC2MS(spawnArgs.GetFloat ( "reloadRate", ".2" ));
...
stateResult_t rvWeaponRocketLauncher::State_Fire ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			...
			Attack ( false, 5, spread, 0, 0.3f ); // Pmod ::  num_attacks :: 1 > 5, power :: 1.0f > 0.3f
